...
image="${BOOK_BUILD_IMAGE:-seal-wallet-book:anonymous-v1}"
target="${1:-package}"
if (( $# > 0 )); then
  shift
fi
...
if [[ -z "$frameworks_repo" ]]; then
  ...
  if [[ -d "$in_repo_candidate/docs/pages/wallet-security" ]]; then
    frameworks_repo="$in_repo_candidate"
  elif [[ -d "$adjacent_candidate/docs/pages/wallet-security" ]]; then
